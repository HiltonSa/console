...
const
  SQL_BASE_ASSOCIADO =
    ...
    '         PAG.PAGAMENTOS, emp.maxpen, COALESCE(vu.ULTREF, C.INIREF) ULTREF, A.COBSUB,                       '+
    '         FNVENC(EMP.ULTREF, C.DIAVENC, EMP.TPVENC, EMP.DIAVENC) DTREC                                      '+
    '  from CONTRATOS C                                                                                         '+
...
const
  SQL_ASSOCIADO = SQL_BASE_ASSOCIADO + ' where C.CODIGO = :CODIGO ) X )';
var
  vQry: TZQuery;
  vResultado: TJsonArray;
begin
...
begin
  ...
  if vPerfil = 4
  then vSQl := Format(SQL_BASE_ASSOCIADO+SQL_PERFIL4,[QuotedStr(Trim(UpperCase(pNome))+'%'), vCidade])
  else vSQl := Format(SQL_BASE_ASSOCIADO+SQL_PERFIL1,[QuotedStr(Trim(UpperCase(pNome))+'%'), pCobrador]);
