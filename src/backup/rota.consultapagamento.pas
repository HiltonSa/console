...
//procedure OnRetornaCarteira(aReq: THorseRequest; aRes: THorseResponse; aNext: TNextProc);
//var
//  vDiaVenc : TDateTime;
//  vResp    : String;
//begin
//  if TryISO8601ToDate(Copy(aReq.Params.Required(True).Field('diavenc').AsString,1,23), vDiaVenc)
//  then vResp := TConsultaPagamentoService.New.Carteira(
//                   aReq.Params.Required(True).Field('cobrador').AsInteger,
//                   vDiaVenc)
//  else vResp := TRetornoJson.New.Sucesso(False).Mensagem('Data Inválida!').ParaString;
